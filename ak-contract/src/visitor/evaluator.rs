    ...
    fn post_visit(&mut self, node: &Node) {
        ...
        match *node {
            ...
            Node::If(_, _, _) => {
                ...
                self.float_stack.push(if cond { then_val } else { else_val });
            }
...
mod tests {
    use super::*;
    use crate::ast::{walk_node, ExprTree, Node};
    ...
    fn eval_f(expr: ExprTree) -> f64 {
        let mut ev = EvaluatorVisitor { variables: vec![], float_stack: Vec::new(), bool_stack: Vec::new() };
    ...
    #[test]
    fn power_operation() {
        let expr = boxed(Node::Pow(boxed(Node::Constant(2.0)), boxed(Node::Constant(3.0))));
        assert_eq!(eval_f(expr), 8.0);
    }

    #[test]
    fn boolean_and() {
        let left = boxed(Node::Equal(boxed(Node::Constant(1.0)), boxed(Node::Constant(1.0))));
        let right = boxed(Node::Different(boxed(Node::Constant(2.0)), boxed(Node::Constant(3.0))));
    ...
    #[test]
    fn subtraction() {
        let expr = boxed(Node::Sub(boxed(Node::Constant(5.0)), boxed(Node::Constant(3.0))));
        assert_eq!(eval_f(expr), 2.0);
    }

    #[test]
    fn multiplication() {
        let expr = boxed(Node::Mul(boxed(Node::Constant(3.0)), boxed(Node::Constant(4.0))));
        assert_eq!(eval_f(expr), 12.0);
    }

    #[test]
    fn division() {
        let expr = boxed(Node::Div(boxed(Node::Constant(10.0)), boxed(Node::Constant(2.0))));
        assert_eq!(eval_f(expr), 5.0);
    }
    ...
    #[test]
    fn max_operation() {
        let expr = boxed(Node::Max(boxed(Node::Constant(2.0)), boxed(Node::Constant(5.0))));
        assert_eq!(eval_f(expr), 5.0);
    }

    #[test]
    fn min_operation() {
        let expr = boxed(Node::Min(boxed(Node::Constant(2.0)), boxed(Node::Constant(5.0))));
        assert_eq!(eval_f(expr), 2.0);
    }

    #[test]
    fn equality() {
        let expr = boxed(Node::Equal(boxed(Node::Constant(2.0)), boxed(Node::Constant(2.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn inequality() {
        let expr = boxed(Node::Different(boxed(Node::Constant(2.0)), boxed(Node::Constant(3.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn superior_comparison() {
        let expr = boxed(Node::Superior(boxed(Node::Constant(5.0)), boxed(Node::Constant(3.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn superior_equal_comparison() {
        let expr = boxed(Node::SuperiorEqual(boxed(Node::Constant(3.0)), boxed(Node::Constant(3.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn inferior_comparison() {
        let expr = boxed(Node::Inferior(boxed(Node::Constant(3.0)), boxed(Node::Constant(5.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn inferior_equal_comparison() {
        let expr = boxed(Node::InferiorEqual(boxed(Node::Constant(3.0)), boxed(Node::Constant(3.0))));
        assert!(eval_b(expr));
    }

    #[test]
    fn boolean_or() {
        let left = boxed(Node::Equal(boxed(Node::Constant(0.0)), boxed(Node::Constant(1.0))));
        let right = boxed(Node::Equal(boxed(Node::Constant(2.0)), boxed(Node::Constant(2.0))));
    ...
    fn variable_lookup() {
        ...
        let mut ev = EvaluatorVisitor { variables: vec![10.0], float_stack: Vec::new(), bool_stack: Vec::new() };
        walk_node(&mut ev, &expr);
    ...
    #[test]
    fn if_statement() {
        let cond = boxed(Node::Equal(boxed(Node::Constant(1.0)), boxed(Node::Constant(1.0))));
