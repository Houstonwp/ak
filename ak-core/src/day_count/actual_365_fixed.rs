...
mod tests {
    ...
    fn test_year_diffs() {
        let cases = vec![
            (date(1992, 2, 1), date(1992, 3, 1), 0.0795),
            (date(1992, 2, 1), date(1993, 3, 1), 1.0795),
            (date(1993, 1, 1), date(1993, 2, 21), 0.1397),
            (date(1993, 1, 1), date(1994, 1, 1), 1.0000),
            (date(1993, 1, 15), date(1993, 2, 1), 0.0466),
            (date(1993, 2, 1), date(1993, 3, 1), 0.0767),
            (date(1993, 2, 1), date(1996, 2, 1), 3.0000),
            (date(1993, 2, 1), date(1993, 3, 1), 0.0767),
            (date(1993, 2, 15), date(1993, 4, 1), 0.1233),
            (date(1993, 3, 15), date(1993, 6, 15), 0.2521),
            (date(1993, 3, 31), date(1993, 4, 1), 0.0027),
            (date(1993, 3, 31), date(1993, 4, 30), 0.0822),
            (date(1993, 3, 31), date(1993, 12, 31), 0.7534),
            (date(1993, 7, 15), date(1993, 9, 15), 0.1699),
            (date(1993, 8, 21), date(1994, 4, 11), 0.6384),
            (date(1993, 11, 1), date(1994, 3, 1), 0.3288),
            (date(1993, 12, 15), date(1993, 12, 30), 0.0411),
            (date(1993, 12, 15), date(1993, 12, 31), 0.0438),
            (date(1993, 12, 31), date(1994, 2, 1), 0.0877),
            (date(1996, 1, 15), date(1996, 5, 31), 0.3753),
            (date(1998, 2, 27), date(1998, 3, 27), 0.0767),
            (date(1998, 2, 28), date(1998, 3, 27), 0.0740),
        ];
        for (start, end, expected) in cases {
            let result = Actual365Fixed.year_diff(start, end);
            assert!(
                (result - expected).abs() < 1e-4,
                "Expected {}, got {}",
                expected,
    ...
    fn test_day_diffs() {
        ...
        for (start, end, expected) in cases {
            let result = Actual365Fixed.day_diff(start, end);
